...
let totalPopulation = populations.reduce((acc, val, i, arr) => acc + val);
...
let myStrongest = monstersInYourPocket.filter((val) => {
    if (val['CP'] > 200){
        return val;
    }
})
...
let ordersTotal = orders.map(val => val.price * (1 + val.tax))
...
let bobsTotal = purchases.filter(val => val.owner === 'Bob').reduce(((acc, val) => acc + val.price), 0);